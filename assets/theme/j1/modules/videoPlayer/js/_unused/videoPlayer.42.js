/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/videoPlayer/js/videoPlayer.js (42)
...
/* Version 3.1.42 for J1 Template */
...
(function (root, factory) {
  ...
  function _resolveNativePosterConfig() {
    ...
    return Object.assign({}, NATIVE_POSTER_DEFAULTS, cfg);
  }
    ...
    return new Promise((resolve) => {
      ...
      const drawFrame = () => {
        try {
          ...
          const dataUrl = canvas.toDataURL(cfg.mimeType, cfg.quality);
          finish(dataUrl);
        } catch (e) {
  ...
  class PlaylistManager {
    ...
    updateEntryPoster(videoId, poster, force) {
      ...
      consoleLog('INFO', MODULE_NAME, `playlistmanager: poster updated for videoId: ${videoId} (${poster.length} bytes)`);
    ...
    generateMissingNativePosters() {
      ...
      isDev && consoleLog('INFO', MODULE_NAME, `playlistmanager: generating posters for ${pending.length} native entr${pending.length === 1 ? 'y' : 'ies'}`);
      ...
      return pending.reduce((chain, entry) => {
        ...
        isDev && consoleLog('INFO', MODULE_NAME, `playlistmanager: generated ${count} native poster(s)`);
        return count;
      });
    }
